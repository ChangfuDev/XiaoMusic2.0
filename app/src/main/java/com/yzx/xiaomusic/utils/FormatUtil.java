...
public class FormatUtil {
    ...
    public static boolean isPassword(String password) {
	    ...
	    总结起来就是第一位必定为1，第二位必定为3或5或8或7，其他位置的可以为0-9
		 */
        String telRegex = "[a-z][0-9a-zA-Z]\\d{5,}";
     ...
     * 加密手机号
     */
    public static String encryptString(String mobileNO) {
        if (mobileNO.length() == 11) {
            return mobileNO.substring(0, 3) + "****" + mobileNO.substring(7, 11);
        }
//        Log.i(Constants.DEBUG_TAG, mobileNO.length() + "");
        return " ";
    }
     ...
     * 加密身份证号
     */
    public static String encryptIdCard(String IdNum) {
        if (IdNum.length() == 15) {
            return IdNum.substring(0, 1) + "*************" + IdNum.substring(IdNum.length() - 1, IdNum.length());
